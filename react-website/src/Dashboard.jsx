  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sheetsRes = await fetch(`/api/sheets-dashboard`);
        ...
        const studentSheets = ['Technical Team', 'CVE Hunt Team', 'Escape Room Team', 'Cyber-AR&VR', 'Internship/Placed', 'Future Participations'];
        ...
        // 2. Build the "levelStats" for the graphs (Hard/Medium/Easy distribution)
        const levelsCount = {};
        uniqueStudents.forEach(s => {
           levelsCount[s.inferredLevel] = (levelsCount[s.inferredLevel] || 0) + 1;
        });
        
        const formattedLevels = Object.keys(levelsCount).map(k => ({
          name: k,
          count: levelsCount[k]
        }));
        setLevelStats(formattedLevels);

        // 3. Set the direct operative listing in dashboard UI
