    ...
    fetch('/api/sheets-dashboard')
      ...
      .then(sheetsData => {
        ...
        const mappedTeams = Array.from(allTeams).map((name, i) => ({
          ...
          technical_summary: `Technical operations inside ${name} division.`,
        ...
        // Projects mapped from the "Project" sheet
        const projData = [];
        if (sheetsData['Project']) {
           sheetsData['Project'].forEach((row, i) => {
             if (row['NAME'] || row['PROJECT NAME']) {
                projData.push({
                   id: i + 1,
